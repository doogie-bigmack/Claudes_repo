...
class APIConfig(BaseSettings):
    ...
    clob_api_url: str = Field(
        default="https://clob.polymarket.com", description="CLOB API for order book"
    )
...
class TradingConfig(BaseSettings):
    ...
    max_position_size: float = Field(
        default=100.0, gt=0, description="Maximum position size per trade in USDC"
    )
    total_capital: float = Field(
        default=1000.0, gt=0, description="Total trading capital in USDC"
    )
    ...
    slippage_tolerance: float = Field(
        default=0.005, ge=0, le=0.1, description="Maximum slippage tolerance"
    )

    # Fees
    taker_fee_rate: float = Field(
        default=0.0315, ge=0, le=0.1, description="Polymarket taker fee rate"
    )
    ...
    gas_estimate: float = Field(
        default=0.001, ge=0, description="Estimated gas cost per trade in MATIC"
    )
...
class RiskConfig(BaseSettings):
    ...
    max_open_positions: int = Field(
        default=10, ge=1, description="Maximum concurrent open positions"
    )
    trade_cooldown: int = Field(
        default=5, ge=0, description="Cooldown between trades in seconds"
    )
    max_trades_per_hour: int = Field(
        default=100, ge=1, description="Maximum trades per hour"
    )
    stop_loss_threshold: float = Field(
        default=0.1, ge=0, le=1.0, description="Stop trading if daily loss exceeds this"
    )
...
class MarketFilterConfig(BaseSettings):
    ...
    min_liquidity: float = Field(
        default=500.0, ge=0, description="Minimum market liquidity in USDC"
    )
    max_time_to_resolution: int = Field(
        default=3600, ge=0, description="Maximum seconds until market resolves"
    )
...
class MonitoringConfig(BaseSettings):
    ...
    metrics_export_interval: int = Field(
        default=60, ge=10, description="Metrics export interval in seconds"
    )
...
class BotConfig(BaseSettings):
    ...
    # Bot operation mode
    dry_run: bool = Field(
        default=True, description="Run in simulation mode without real trades"
    )
